...
def show_convert_library_page():
    target_formats_value = CWA_DB().cwa_settings['auto_convert_target_format']
    if isinstance(target_formats_value, str):
        target_formats_value = [target_formats_value]
    return render_title_template('cwa_convert_library.html', title=_("Calibre-Web Automated - Convert Library"), page="cwa-library-convert",
                                target_format=', '.join(f.upper() for f in target_formats_value))
